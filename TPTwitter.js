			...
			if(reply){
				client.hget('user:'+id, 'password', function(err, reply){
					...
					if(reply && password == req.body.passwd){
						...
						res.render('LogResult.ejs');
					}else{
						console.log('fail login');
						//prevoir msg erreur
						res.render('accueil.ejs', {msg: 'Echec de la connexion'});
					}
				});
			}else{
				res.render('accueil.ejs', {msg: 'Echec de la connexion'});
...
app.post('/tptwitter/newuser', urlencodedParser, function(req, res){
	...
	if(req.body.newlogin !='' && req.body.newpasswd != '' && req.body.newpasswd2 != ''){
		...
		if(req.body.newpasswd != req.body.newpasswd2){
			...
			res.render('accueil.ejs', {msg2: 'Mots de passes différents'});
		}else{
			var id;
			...
			client.get('userid', function(err, reply){
				if(reply){
					...
					res.render('accueil.ejs', {msg2: 'Echec de l\'inscription, veuillez rééssayer plus tard'});
				}
				...
				res.render('accueil.ejs', {msg2: 'Vous êtes incrit, vous pouvez desormais vous connecter :)'});
			});
		}
